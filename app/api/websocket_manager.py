...
class ConnectionManager:
    ...
    async def broadcast_positions(self, positions: Dict[str, Any]):
        ...
        logger.info(f"Broadcasting {len(positions)} position updates to {len(connections)} connected clients")
        ...
        if disconnected:
            with self._lock:
                ...
                logger.info(f"Removed {len(disconnected)} disconnected clients. {len(self.active_connections)} remaining.")